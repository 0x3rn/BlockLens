...
The website offers three analysis horizons: Short-term (6 hours–3 days: 15m, 1H, 4H, 1D), Swing (3 days–4 weeks: 4H, 1D, 1W), and Long-term (1–12+ months: 1D, 1W, 1M). The server resolves verified spot pairs from CoinGecko ticker metadata, prefers Binance, and falls back to Coinbase Exchange when the deployment cannot reach Binance. It excludes open candles, validates OHLCV invariants, and fails explicitly when complete coverage is unavailable. Exchange-candle analysis currently requires USD display currency.

...
The website offers three analysis horizons: Short-term (6 hours–3 days: 15m, 1H, 4H, 1D), Swing (3 days–4 weeks: 4H, 1D, 1W), and Long-term (1–12+ months: 1D, 1W, 1M). The server resolves a verified Binance Spot USDT pair from CoinGecko ticker metadata, excludes the current open candle, validates OHLCV invariants, and fails explicitly when complete coverage is unavailable. Exchange-candle analysis currently requires USD display currency.
...
api/_market.ts              Shared CoinGecko snapshot, verified Binance pair resolution, and closed-candle loader
